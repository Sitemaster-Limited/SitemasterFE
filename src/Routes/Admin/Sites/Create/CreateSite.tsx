...
const CreateSite = () => {
  ...
  useEffect(() => {
    ...
    updateFormData({ email: user.user?.primaryEmailAddress?.emailAddress});
  }, []);
  ...
  const handleSiteGeneration = async (siteId: string, client: string, files: File[], token: string, type: string) => {
    ...
    try {
      ...
      await PostImages(siteId, client, files, [file], token, type);

      const qrCode = await GetImages(siteId, client, "QRCode");
      const blueprints = await GetImages(siteId, client, "Blueprints");
      apiData.siteInfo.qrCode = qrCode.preSignedUrls;
      apiData.siteInfo.bluePrints = blueprints.preSignedUrls;
        ...
        <div className="flex justify-center items-center">
          {showQR && (
            <div className="flex flex-col justify-center items-center">
              ...
              <div className="w-[250px] h-[250px] inline-block mb-4" ref={qrCanvasRef}>
                <QRCode
