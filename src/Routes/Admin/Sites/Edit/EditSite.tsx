...
const EditSite = () => {
  ...
  const handleDownloadQRCode = async () => {
    if (qrImageRef.current) {
      const canvas = await html2canvas(qrImageRef.current, { useCORS: true });
      const image = canvas.toDataURL('image/png');
      const link = document.createElement('a');
      link.href = image;
      link.download = 'custom-site-qr-code.png';
      link.click();
    }
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 p-2">
        ...
        <div className="order-1 md:order-2 flex flex-col items-center md:px-6">
          ...
          <div className="flex flex-col w-full justify-center items-center mt-4">
            <div ref={qrImageRef}
                 className="flex flex-col justify-center items-center w-full bg-white p-4 shadow-md rounded-md">
              <h1 className="font-semibold text-2xl mb-4">Sitemaster</h1>
              <h2 className="font-semibold text-xl mb-4">Custom Site QR Code:</h2>
              <img
                src={String(localSite?.siteInfo.qrCode)}
                alt="QRCode"
                className="w-[250px] h-[250px] mb-4"
              />
              <footer className="text-gray-600 text-sm mt-4">Generated by Sitemaster</footer>
            </div>
            <button
